...
class Interfaz:
    def __init__(self, ventana):
        #Iniciar la ventana con un título
        self.ventana = ventana
        self.ventana.title("Calculadora")

        #Agregar una caja de texto para que sea la pantalla de la calculadora
        self.pantalla = Text(ventana, state = "disabled", width = 40, height = 3, background ="orchid" , 
        foreground = "white", font = ("Helvetica", 15) )

        #Ubicar la pantalla en la ventana
        self.pantalla.grid(row = 0, column = 0, columnspan = 4, padx = 5, pady = 5)

        #Inicializar la operacion mostrada en pantalla como string vacío
        self.operacion  = ""
        ...
        for fila in range(1,5):
            for columna in  range (4):
                ...
                contador+= 1
        #UBicar e ultimo boton al final
    ...
    def click(self, texto, escribir):
        ...
        if not escribir:
            if texto == "=" and self.operacion != "":
                #Reemplazar el valor inocode de la division por el operador divisiion de Python '/'
                self.operacion = re.sub(u"\u00F7","/", self.operacion)
                resultado = str(eval(self.operacion))
                self.operacion= ""
                self.limpiarPantalla()
                self.mostrarEnPantalla(resultado)
            #Si se presionó el lbotón de borrado, limpiar la pantalla
            elif texto == u"\u232B":
                self.operacion  = ""
                self.limpiarPantalla()
        #Mostrar texto
